...
class TrainDataLoaderJoie(TrainDataLoader):
    ...
    def __init__(self, kg_instance, kg_ontology, kg_ins_type, example2idx, n_batches, sampling_type, use_cuda=None):
        ...
        instance_head_idx, instance_tail_idx = (reindex_kg_idx_enteties_vector(kg_instance.head_idx, kg_instance, example2idx),
                                                reindex_kg_idx_enteties_vector(kg_instance.tail_idx, kg_instance, example2idx))
        ontology_head_idx, ontology_tail_idx = (reindex_kg_idx_enteties_vector(kg_ontology.head_idx, kg_ontology, example2idx),
                                                reindex_kg_idx_enteties_vector(kg_ontology.tail_idx, kg_ontology, example2idx))
        ins_type_head_idx, ins_type_tail_idx = (reindex_kg_idx_enteties_vector(kg_ins_type.head_idx, kg_ins_type, example2idx),
...
class TrainerJoie(Trainer):
    ...
    def run(self,off_cross_gap = 0,eps = 0.5):
        ...
        for epoch in iterator:
            ...
            iterator.set_description(
                'Epoch {} | Average losses: joie loss: {:.5f} | loss intra view intstance model: {:.5f} | loss intra ontology model: {:.5f} | loss cross view model: {:.5f}'.format(epoch + 1, joie_loss, losses['instance'], losses['ontology'], losses['type']))
    ...
    def plot_train_process(self):
        ...
        for i in range(3):
   	    	...
   	    	axes[i].plot(list(range(len(x)-1)),x[1:])
   	    	axes[i].grid()
