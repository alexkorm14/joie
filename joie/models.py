...
class JOIE:
    ...
    def kg_index_dicts(self,kg_instance,kg_ontology,kg_ins_type):
        ...
        enteties_from_instances = list(kg_instance.ent2ix.keys())
        enteties_from_instype = kg_ins_type.get_df()['from'].unique().tolist()
        enteties = sorted(set(enteties_from_instances) | set(enteties_from_instype))
    ...
    def init_models(self,lr_cross = 0.1,lr_instance = 0.02,lr_ontology=0.01):
        ...
        intra_view_instance_model = intra_models[self.type_intra_models.lower()](emb_dim = self.entity_dim, 
                                                                n_entities = self.n_ent, n_relations = self.n_rel_ent)
        intra_view_ontology_model = intra_models[self.type_intra_models.lower()](emb_dim = self.concept_dim, 
                                                                n_entities = self.n_con, n_relations = self.n_rel_con)
        ...
        optimizer_intra_instance = torch.optim.Adam(intra_view_instance_model.parameters(),lr = lr_instance,amsgrad=True)
        optimizer_intra_ontology = torch.optim.Adam(intra_view_ontology_model.parameters(),lr = lr_ontology,amsgrad=True)
        # Что здесь происходит ОЛЬГА
        cross_model.concept_embeddings.weight.requires_grad = False
        optimizer_cross = torch.optim.Adam(filter(lambda p: p.requires_grad, cross_model.parameters()),lr = lr_cross,amsgrad=True)
